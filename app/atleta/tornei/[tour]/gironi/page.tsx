...
/** Etichetta finale risolta (se token G1/G2) */
function displayTeamLabel(L: string, raw: string, data: Persist): string {
  return /^(Vincente|Perdente)\s+G[12]$/i.test(raw) ? resolvePoolToken(L, raw, data) : raw
}
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="md:hidden">
            ...
            <div className="flex justify-center gap-2 mb-3">
              {letters.map((L, i) => (
                <button
                  key={L}
                  onClick={()=>scrollTo(i)}
                  className={['h-2.5 w-2.5 rounded-full transition-opacity', i===currentIdx ? 'bg-white opacity-100' : 'bg-neutral-500 opacity-50'].join(' ')}
            ...
            {/* carosello */}
            <div className="snap-x snap-mandatory overflow-x-auto -mx-4 px-4" onScroll={onScroll}>
                            ...
                            <div className="inline-block min-w-[640px] w-full">
                              <div className="space-y-2">
                                {rows.length===0 ? (
                                  ...
                                  <div key={idx} className="grid items-center whitespace-nowrap"
                                       style={{gridTemplateColumns:'70px minmax(170px,1fr) 44px 18px 44px minmax(170px,1fr)', columnGap:'.45rem'}}>
                                    <div className="input h-8 pl-1 pr-0 text-sm tabular-nums">{(data.times?.[L]?.[idx] ?? '') || '—'}</div>
                                    <div className="min-w-0 truncate text-sm text-right">{displayTeamLabel(L, r.t1, data)}</div>
                                    ...
                                    <div className="min-w-0 truncate text-sm pl-1">{displayTeamLabel(L, r.t2, data)}</div>
                                  </div>
                                ))}
                              </div>
                            </div>
          ...
          {/* ===== DESKTOP: due colonne; “Partite {L}” larga come due gironi ===== */}
          <div className="hidden md:grid gap-4 grid-cols-2 auto-rows-max">
            {letters.map((L) => {
              const m = data.meta?.[L] ?? { capacity: 0, format: 'pool' as const }
              const cap = m.capacity ?? 0
              const rows = scheduleRows(L, data)
