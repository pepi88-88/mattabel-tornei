...
export default function PagamentiPage() {
  ...
  const visibleSorted = useMemo(() => {
    return visibleFiltered
      ...
      .sort((x, y) => {
        ...
        if (doneX !== doneY) return doneX - doneY
        return x.idx - y.idx
      })
      .map(x => x.r)
  }, [visibleFiltered])
  ...
  // totali (su TUTTE le squadre da pagare, indipendente da ricerca)
  const stats = useMemo(() => {
    let a = 0,
      b = 0,
      both = 0
  ...
  // 1) filtra per ricerca su nome A o B
  const visibleFiltered = useMemo(() => {
    const q = normalize(search)
    if (!q) return payableItems
    return payableItems.filter((r: any) => {
  ...
  // 2) ordina: prima squadre NON complete, poi quelle con Pagato A+B in fondo
  const visibleSorted = useMemo(() => {
    return visibleFiltered
      .map((r: any, idx: number) => ({ r, idx }))
      .sort((x, y) => {
        const doneX = x.r.paid_a && x.r.paid_b ? 1 : 0
        const doneY = y.r.paid_a && y.r.paid_b ? 1 : 0
        if (doneX !== doneY) return doneX - doneY // 0 (non complete) prima, 1 (complete) dopo
        return x.idx - y.idx // mantiene l'ordine originale dentro al gruppo
      })
      .map(x => x.r)
  }, [visibleFiltered])
  ...
  return (
    <div className="space-y-6 p-6">
      {/* intestazione: stessi selettori di Iscrizioni + ricerca + stats */}
      <div className="flex flex-wrap items-end gap-3">
        <div className="flex flex-col gap-1">
          <span className="text-sm text-neutral-400">Tour</span>
          <select
